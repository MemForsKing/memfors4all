...
class PoolScanFilef(scan.PoolScanner):
...
class FileScanf(common.AbstractWindowsCommand):
    ...
    def calculate(self):
        ...
        for offset in PoolScanFilef().scan(address_space):
    ...
    def render_text(self, outfd, data):
        ...
        outfd.write("{0}||{1}||{2}||{3}||{4}\n".format('Offset(P)', '#Ptr', '#Hnd', 'Access', 'Name'))

        for object_obj, file_obj in data:
            outfd.write("{0:#x}||{1}||{2}||{3}||{4}\n".format(
...
class PoolScanDriverf(PoolScanFilef):
...
class DriverScanf(FileScanf):
    ...
    def calculate(self):
        ...
        for offset in PoolScanDriverf().scan(address_space):
            pool_obj = obj.Object("_POOL_HEADER", vm = address_space,
                                 offset = offset)
    ...
    def render_text(self, outfd, data):
        ...
        outfd.write("{0}||{1}||{2}||{3}||{4}||{5}||{6}||{7}\n".format('Offset(P)', '#Ptr', '#Hnd', 'Start', 'Size', 'Service Key', 'Name', 'Driver Name'))

        for object_obj, driver_obj, extension_obj in data:

            outfd.write("{0:#x}||{1}||{2}||{3:#x}||{4:#x}||{5}||{6}||{7}\n".format(
...
class PoolScanSymlinkf(PoolScanFilef):
...
class SymLinkScanf(FileScanf):
    ...
    def calculate(self):
        ...
        for offset in PoolScanSymlinkf().scan(address_space):
            pool_obj = obj.Object("_POOL_HEADER", vm = address_space,
                                 offset = offset)
    ...
    def render_text(self, outfd, data):
        ...
        outfd.write("{0}||{1}||{2}||{3}||{4}||{5}\n".format('Offset(P)', '#Ptr', '#Hnd', 'Creation time', 'From', 'To'))

        for objct, link in data:
            outfd.write("{0:#x}||{1}||{2}||{3}||{4}||{5}\n".format(link.obj_offset, objct.PointerCount,
                        objct.HandleCount, link.CreationTime or '',
                        str(objct.NameInfo.Name or ''),
                        str(link.LinkTarget or '')))

class PoolScanMutantf(PoolScanDriverf):
...
class MutantScanf(FileScanf):
    "Scan for mutant objects _KMUTANT "
    def __init__(self, config, *args, **kwargs):
        FileScanf.__init__(self, config, *args, **kwargs)
    ...
    def calculate(self):
        ...
        for offset in PoolScanMutantf().scan(address_space):
            pool_obj = obj.Object("_POOL_HEADER", vm = address_space,
                                 offset = offset)
    ...
    def render_text(self, outfd, data):
        ...
        outfd.write("{0}||{1}||{2}||{3}||{4}||{5}||{6}\n".format('Offset(P)', '#Ptr', '#Hnd', 'Signal', 'Thread', 'CID', 'Name'))
        ...
        for object_obj, mutant in data:
            ...
            outfd.write("{0:#x}||{1}||{2}||{3}||{4:#x}||{5}||{6}\n".format(mutant.obj_offset, object_obj.PointerCount,
                         object_obj.HandleCount, mutant.Header.SignalState,
                         mutant.OwnerThread, CID,
                         str(object_obj.NameInfo.Name or '')
                         ))

class CheckProcessf(scan.ScannerCheck):
...
class PoolScanProcessf(scan.PoolScanner):
    """PoolScanner for File objects"""
...
class PSScanf(common.AbstractWindowsCommand):
    ...
    def calculate(self):
        ...
        for offset in PoolScanProcessf().scan(address_space):
    ...
    def render_text(self, outfd, data):
        ...
        outfd.write("{0}||{1}||{2}||{3}||{4}||{5}||{6}\n".format('Offset(P)', 'Name', 'PID', 'PPID', 'PDB', 'Time Created', 'Time Exited'))
